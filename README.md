...
---
#### 最新版本v2.0.5 (2026.9.8)
- 1、修复子目录ID重复导致一级目录切换残留的BUG
- 2、新增数据迁移函数，自动检测并修复重复的子目录ID
- 3、切换到"置顶网站"时正确清除子目录选中状态
- 4、页面左下角版本号更新至v2.0.5

#### 版本v2.0.4 (2026.9.7)
- 1、彻底移除 ensureSameNameSubCategories 函数，解决切换一级目录时内容残留的问题
- 2、不再在加载数据时修改分类结构和链接归属
- 3、页面左下角版本号更新至v2.0.4

#### 版本v2.0.1~v2.0.3 (2026.9.7)
- 多次尝试修复同名子目录问题，逐步回退

#### 版本v2.0.0 (2026.9.7)
- 1、新增多地址功能：支持为书签添加多个备用地址（内网/外网/IPv6等）
- 2、右键菜单直接显示备用地址，点击即可在新标签页打开
- 3、编辑链接弹窗新增备用地址管理
- 4、页面左下角版本号更新至v2.0.0
...
#### 版本v1.9.8 (2026.9.7)
- 1、修复覆盖导入时分类丢失导致书签全部变成未分类的问题
- 2、覆盖导入改为使用文件中的完整分类数据
- 3、页面左下角版本号更新至v1.9.8

#### 版本v1.9.7 (2026.9.7)
- 1、修复「未分类」目录没有批量编辑按钮的问题
- 2、页面左下角版本号更新至v1.9.7

#### 版本v1.9.6 (2026.9.7)
- 1、修复孤立链接（categoryId指向不存在的子目录）无法显示的问题
- 2、侧边栏新增「未分类」虚拟分类，自动收集所有孤立链接
- 3、回退v1.9.5中过于激进的自动清理逻辑，避免误删数据
- 4、页面左下角版本号更新至v1.9.6

#### 版本v1.9.5 (2026.9.7)
- 1、修复删除文件夹（子目录）后，对应书签未被清理的问题（书签自动移至上级目录）
- 2、导入备份新增「覆盖导入」模式，可完全替换现有数据（原增量导入仍保留为默认选项）
- 3、新增「全部清空」功能（设置 → 扩展工具 → 数据管理），方便一键重置所有数据
- 4、页面左下角版本号更新至v1.9.5
