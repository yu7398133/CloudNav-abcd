...
function App() {
  ...
  const handleImportConfirm = (newLinks: LinkItem[], newCategories: Category[], overwrite?: boolean) => {
      ...
      // Merge categories: Avoid duplicate names/IDs, handle subcategories
      ...
      newCategories.forEach(nc => {
          const existingIdx = mergedCategories.findIndex(c => c.id === nc.id || c.name === nc.name);
          if (existingIdx === -1) {
              // Completely new category
              mergedCategories.push(nc);
          } else if (nc.subCategories && nc.subCategories.length > 0) {
              // Category exists but has new subcategories to merge
              const existing = mergedCategories[existingIdx];
              const existingSubNames = new Set(
                  (existing.subCategories || []).map(s => s.name)
              );
              const newSubs = nc.subCategories.filter(s => !existingSubNames.has(s.name));
              if (newSubs.length > 0) {
